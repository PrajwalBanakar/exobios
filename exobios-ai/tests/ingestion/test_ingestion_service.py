...
def test_ingest_txt_file_end_to_end(tmp_path: Path):
    ...
    service = build_default_ingestion_service(chunk_size=200, chunk_overlap=20)
...
def test_checksum_generation_is_deterministic_for_identical_content(tmp_path: Path):
    file_path = tmp_path / "note.txt"
...
def test_duplicate_content_is_rejected_even_under_a_different_filename(tmp_path: Path):
...
def test_document_is_registered_and_retrievable(tmp_path: Path):
    file_path = tmp_path / "note.txt"
