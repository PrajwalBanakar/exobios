...
import { isThisMonth } from '@/shared/utils/format'
    ...
    <div class="p-4 md:p-6 space-y-5">
      ...
      <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard v-for="stat in [
          { label: 'Total Feedback', val: feedbackList.length,                                        tone: 'blue'  },
          { label: 'This Month',     val: feedbackList.filter(f => isThisMonth(f.date)).length,        tone: 'blue'  },
          { label: 'Responded',      val: feedbackList.filter(f => f.responded).length,                tone: 'green' },
          { label: 'Avg Rating',     val: avgRating + ' ★',                                            tone: 'amber' },
        ]" :key="stat.label" :label="stat.label" :value="stat.val" :tone="stat.tone">
          <template #icon>
            <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>
          </template>
        </StatCard>
      </div>
      ...
      <div class="bg-white rounded-xl border border-slate-100 shadow-sm">
        <div class="flex items-center justify-between px-5 py-4 border-b border-slate-100 gap-3 flex-wrap">
          <h2 class="font-semibold text-slate-900 text-sm">{{ t('feedback.allTypes') }}</h2>
        ...
        <EmptyState v-if="filtered.length === 0" icon="folder" :title="t('feedback.noFeedback')" message="Feedback submitted here will appear in this list."/>

        <div v-else class="divide-y divide-slate-50">
          <div v-for="fb in filtered" :key="fb.id" class="px-5 py-4 hover:bg-slate-50 transition">
            <div class="flex items-start gap-4">
              ...
              <div class="flex-1 min-w-0">
                <div class="flex items-center gap-2 flex-wrap">
                  <span class="font-medium text-slate-800 text-sm">{{ fb.from }}</span>
                  <span class="bg-blue-100 text-blue-600 px-2 py-0.5 text-xs font-medium rounded">{{ fb.category }}</span>
                </div>
                <div class="flex items-center gap-1 mt-1">
                  <span v-for="s in 5" :key="s" :class="['text-base', s <= fb.rating ? 'text-amber-400' : 'text-slate-200']">★</span>
                  <span :class="[ratingColor(fb.rating), 'text-xs font-semibold ml-1']">{{ fb.rating }}/5</span>
                </div>
                <p class="text-sm text-slate-600 mt-1.5 leading-relaxed">{{ fb.comment }}</p>
                <div class="text-xs text-slate-400 mt-1">{{ fb.date }}</div>
              </div>
      ...
      <div v-if="showModal" class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" @click.self="showModal=false">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg">
          <div class="flex items-center justify-between px-6 py-4 border-b border-slate-100">
            <h3 class="font-semibold text-slate-900 text-lg">{{ t('feedback.giveFeedback') }}</h3>
          ...
          <div class="px-6 py-5 space-y-4">
            <div>
              <label class="block text-xs font-medium text-slate-600 mb-1.5">{{ t('feedback.type') }}</label>
              <select v-model="form.category" class="w-full border border-slate-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white">
                <option v-for="c in CATEGORIES" :key="c" :value="c">{{ c }}</option>
              </select>
            </div>
            <div>
              <label class="block text-xs font-medium text-slate-600 mb-1.5">{{ t('feedback.rating') }} <span class="text-red-500">*</span></label>
              <div class="flex items-center gap-1">
                <button v-for="s in 5" :key="s" @click="form.rating=s" :class="['text-3xl transition-transform hover:scale-110', s <= form.rating ? 'text-amber-400' : 'text-slate-200']">★</button>
                <span class="ml-2 text-sm text-slate-500">{{ form.rating > 0 ? form.rating + '/5' : '' }}</span>
              </div>
            </div>
            <div>
              <label class="block text-xs font-medium text-slate-600 mb-1.5">{{ t('feedback.comment') }} <span class="text-red-500">*</span></label>
              <textarea v-model="form.comment" rows="3" :placeholder="t('feedback.commentPlaceholder')"
                class="w-full border border-slate-200 rounded-xl px-3 py-2 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-blue-500"/>
            </div>
            <label class="flex items-center gap-2 cursor-pointer">
              <input v-model="form.anonymous" type="checkbox" class="w-4 h-4 rounded border-slate-300 text-blue-600"/>
              <span class="text-sm text-slate-600">{{ t('feedback.anonymous') }}</span>
            </label>
          </div>
