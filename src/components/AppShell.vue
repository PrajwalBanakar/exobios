...
const nav = computed(() => [
  ...
  { key: 'nav.patients',    icon: 'users',      to: '/patients' },
  { key: 'nav.sos',         icon: 'sos',        to: '/sos',        badge: true },
  { key: 'nav.referrals',   icon: 'referral',   to: '/referrals' },
  { key: 'nav.teleconsult', icon: 'video',      to: '/teleconsult' },
  { key: 'nav.reports',     icon: 'chart',      to: '/reports' },
  ...(auth.canManageUsers ? [{ key: 'nav.users', icon: 'person', to: '/users' }] : []),
...
function updateSyncTime() {
  const now = new Date()
  syncTime.value = now.toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' }) +
...
onMounted(() => {
  updateSyncTime()
  syncInterval = setInterval(updateSyncTime, 60000)
    ...
    <aside data-sidebar
      ...
      <nav class="flex-1 px-3 py-2 space-y-0.5 overflow-y-auto">
        ...
        >
          ...
          <!-- Referral (hospital) -->
          <svg v-else-if="item.icon === 'referral'" class="w-5 h-5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8">
            <path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/>
          </svg>
          <!-- Video (teleconsult) -->
          <svg v-else-if="item.icon === 'video'" class="w-5 h-5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8">
      ...
      <div class="px-3 pb-4 flex-shrink-0">
        <router-link to="/assessment/new" @click="closeSidebar"
      ...
      <header class="h-14 bg-white border-b border-gray-200 flex items-center justify-between px-4 md:px-6 flex-shrink-0">
        <div class="flex items-center gap-3 min-w-0">
          ...
          <slot name="topbar-left">
            <div class="min-w-0">
              ...
              <p class="text-xs text-gray-400 hidden sm:block truncate"><slot name="page-subtitle"></slot></p>
            </div>
          </slot>
        </div>
        ...
        <div class="flex items-center gap-2 md:gap-4 flex-shrink-0">
          ...
          <div class="hidden md:flex items-center gap-1.5 text-xs"
            :class="isOnline ? 'text-gray-500' : 'text-orange-500'">
            <span :class="['w-2 h-2 rounded-full', isOnline ? 'bg-green-500' : 'bg-orange-500 animate-pulse']"></span>
          ...
          <div class="relative hidden sm:block" data-popup>
            ...
            <div v-if="showLangMenu"
              class="absolute right-0 top-full mt-2 w-36 bg-white rounded-xl shadow-xl border border-gray-100 z-50 overflow-hidden py-1">
          ...
          <div class="relative" data-popup>
            ...
            <div v-if="showNotifications"
              ...
              <div class="px-4 py-3 border-b border-gray-100 flex items-center justify-between">
                ...
                <button v-if="notifStore.unreadCount > 0"
              ...
              <div class="max-h-72 overflow-y-auto">
                <div v-for="n in notifStore.items.slice(0, 5)" :key="n.id"
                  ...
                  <span :class="[notifDotColor[n.type], 'w-2 h-2 rounded-full mt-1.5 flex-shrink-0']"></span>
                  ...
                  <span v-if="!n.read" class="w-2 h-2 rounded-full bg-blue-500 mt-1.5 flex-shrink-0"></span>
                </div>
          ...
          <!-- User Profile -->
          <div class="relative" data-popup>
            <button class="flex items-center gap-2 cursor-pointer p-1 rounded-lg hover:bg-gray-50 transition"
              ...
              <svg class="w-3.5 h-3.5 text-gray-400 flex-shrink-0 hidden sm:block" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
            ...
            <div v-if="showProfileMenu"
              class="absolute right-0 top-full mt-2 w-44 bg-white rounded-xl shadow-xl border border-gray-100 z-50 overflow-hidden">
              <button
              ...
              <button
                class="w-full flex items-center gap-3 px-4 py-3 text-sm text-red-600 hover:bg-red-50 transition text-left"
                @click="logout">
