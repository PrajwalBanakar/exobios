...
const statusColor = (s) => s === 'Active' ? 'bg-green-100 text-green-700' : s === 'Inactive' ? 'bg-slate-100 text-slate-500' : 'bg-red-100 text-red-600'
    ...
    <div class="p-4 md:p-6 space-y-5">
      ...
      <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
        <div v-for="role in ROLES" :key="role"
          :class="['bg-white rounded-xl border border-slate-100 shadow-sm p-3 flex items-center gap-3 cursor-pointer transition hover:shadow-md', roleFilter===role ? 'ring-2 ring-blue-400' : '']"
          ...
          <div class="w-9 h-9 rounded-full bg-slate-100 flex items-center justify-center flex-shrink-0">
            <svg class="w-4 h-4 text-slate-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8"><circle cx="12" cy="8" r="4"/><path d="M4 20c0-4 3.6-7 8-7s8 3 8 7"/></svg>
          </div>
          <div>
            <div class="text-lg font-bold text-slate-900">{{ roleCount[role] }}</div>
            <div class="text-[11px] text-slate-500 leading-tight">{{ role }}</div>
          </div>
        </div>
      </div>
      ...
      <div class="bg-white rounded-xl border border-slate-100 shadow-sm">
        ...
        <div class="flex items-center justify-between gap-3 px-5 py-4 border-b border-slate-100 flex-wrap">
          <div class="flex items-center gap-2 flex-1 min-w-0">
            <div class="relative flex-1 max-w-xs">
              <svg class="w-4 h-4 text-slate-400 absolute left-3 top-1/2 -translate-y-1/2" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/></svg>
              <input v-model="searchQuery" type="text" placeholder="Search users…" class="pl-9 pr-3 py-2 w-full text-sm border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500"/>
            </div>
            <select v-model="roleFilter" class="text-sm border border-slate-200 rounded-xl px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white">
              <option value="All">{{ t('admin.allRoles') }}</option>
              <option v-for="r in ROLES" :key="r" :value="r">{{ r }}</option>
            </select>
          </div>
          <button @click="openAdd" class="flex items-center gap-1.5 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-semibold rounded-xl transition flex-shrink-0">
        ...
        <div v-if="filtered.length === 0" class="py-16 text-center">
          <svg class="w-12 h-12 text-slate-200 mx-auto mb-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.2"><circle cx="12" cy="8" r="4"/><path d="M4 20c0-4 3.6-7 8-7s8 3 8 7"/></svg>
            ...
            <thead class="bg-slate-50 text-xs text-slate-500 font-medium">
              <tr>
                <th v-for="col in [{ key:'name', label:'Name' }, { key:'email', label:t('admin.email') }, { key:'phone', label:t('admin.phone') }, { key:'role', label:t('admin.roles') }, { key:'status', label:t('admin.status') }, { key:'lastLogin', label:t('admin.lastLogin') }]" :key="col.key"
                  class="px-4 py-3 text-left cursor-pointer hover:text-slate-700 select-none"
                  @click="toggleSort(col.key)">
                  <span class="flex items-center gap-1">{{ col.label }} <span class="text-slate-400">{{ sortIcon(col.key) }}</span></span>
                </th>
                <th class="px-4 py-3 text-left">{{ t('admin.actions') }}</th>
              </tr>
            </thead>
            <tbody class="divide-y divide-slate-50">
              <tr v-for="user in filtered" :key="user.id" class="hover:bg-slate-50 transition">
                <td class="px-4 py-3">
                ...
                <td class="px-4 py-3 text-slate-500 text-xs">{{ user.lastLogin }}</td>
                <td class="px-4 py-3">
                  <div class="flex items-center gap-2">
                    <button @click="openEdit(user)" class="p-1.5 text-blue-600 hover:bg-blue-50 rounded-xl transition">
                    ...
                    <button @click="confirmDelete(user.id)" class="p-1.5 text-red-500 hover:bg-red-50 rounded-xl transition">
        ...
        <div v-if="filtered.length" class="px-5 py-3 border-t border-slate-100 text-xs text-slate-400">
    ...
    <teleport to="body">
      <div v-if="showModal" class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" @click.self="showModal=false">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md">
          <div class="flex items-center justify-between px-6 py-4 border-b border-slate-100">
            <h3 class="font-semibold text-slate-900 text-lg">{{ isEditing ? t('admin.editUser') : t('admin.addUser') }}</h3>
          ...
          <div class="px-6 py-5 space-y-4">
            <div>
              ...
              <input v-model="form.name" type="text" class="w-full border border-slate-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"/>
            </div>
            <div>
              <label class="block text-xs font-medium text-slate-600 mb-1">{{ t('admin.email') }} <span class="text-red-500">*</span></label>
              <input v-model="form.email" type="email" class="w-full border border-slate-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"/>
            </div>
            <div>
              <label class="block text-xs font-medium text-slate-600 mb-1">{{ t('admin.phone') }}</label>
              <input v-model="form.phone" type="tel" class="w-full border border-slate-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"/>
            </div>
            <div class="grid grid-cols-2 gap-3">
              <div>
                <label class="block text-xs font-medium text-slate-600 mb-1">{{ t('admin.roles') }}</label>
                <select v-model="form.role" class="w-full border border-slate-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white">
                  <option v-for="r in ROLES" :key="r" :value="r">{{ r }}</option>
                </select>
              </div>
              <div>
                <label class="block text-xs font-medium text-slate-600 mb-1">{{ t('admin.status') }}</label>
                <select v-model="form.status" class="w-full border border-slate-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white">
          ...
          <div class="flex items-center justify-end gap-3 px-6 py-4 border-t border-slate-100">
            <button @click="showModal=false" class="px-4 py-2 text-sm border border-slate-200 text-slate-600 rounded-xl hover:bg-slate-50">{{ t('common.cancel') }}</button>
      ...
      <div v-if="deleteId !== null" class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" @click.self="deleteId=null">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 text-center">
          ...
          <h3 class="font-semibold text-slate-900 text-lg mb-1">Delete User</h3>
          <p class="text-sm text-slate-500 mb-5">This action cannot be undone. The user will be permanently removed.</p>
          <div class="flex gap-3">
            <button @click="deleteId=null" class="flex-1 py-2 text-sm border border-slate-200 text-slate-600 rounded-xl hover:bg-slate-50">Cancel</button>
            <button @click="deleteUser" class="flex-1 py-2 text-sm bg-red-600 hover:bg-red-700 text-white font-semibold rounded-xl transition">Delete</button>
          </div>
        </div>
      </div>
    </teleport>
