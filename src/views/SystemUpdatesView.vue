...
const filtered = computed(() => {
  if (typeFilter.value === 'All') return updates.value
  return updates.value.filter(u => u.type === typeFilter.value.toLowerCase())
})

const unread = computed(() => updates.value.filter(u => !u.read).length)

function markAllRead() {
  updates.value.forEach(u => u.read = true)
}
    ...
    <div class="p-6 space-y-5">
      ...
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-3">
          ...
          <span v-if="unread > 0" class="px-2.5 py-1 bg-red-100 text-red-600 text-xs font-semibold rounded-full">
            {{ unread }} unread
          </span>
        </div>
        <button v-if="unread > 0" @click="markAllRead"
          class="text-xs text-blue-600 hover:underline font-medium">Mark all as read</button>
      </div>

      <!-- Filter pills -->
      <div class="flex items-center gap-2">
        <button v-for="t in types" :key="t"
          :class="['px-3 py-1.5 rounded-full text-xs font-medium transition', typeFilter===t ? 'bg-blue-600 text-white' : 'bg-white border border-gray-200 text-gray-600 hover:border-blue-300']"
      ...
      <div class="space-y-3">
        <div v-for="u in filtered" :key="u.id"
          :class="['bg-white rounded-xl border shadow-sm px-5 py-4 flex items-start gap-4 transition', u.read ? 'border-gray-100' : 'border-blue-200 bg-blue-50/20']"
          @click="markRead(u.id)">
          ...
          <div class="flex-1 min-w-0">
            <div class="flex items-start justify-between gap-3">
              ...
              <div class="flex items-center gap-2 flex-shrink-0">
                ...
                <span v-if="!u.read" class="w-2 h-2 rounded-full bg-blue-500"></span>
              </div>
            </div>
