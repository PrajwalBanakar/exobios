...
const conditions = [
  {
    name: 'Dengue Fever', pct: 72, confidence: 'High',
    evidence: ['High fever (101.2°F)', 'Body aches and joint pain', 'Platelet count concern', 'Rash pattern consistent'],
    risk: 'High', icd: 'A90',
  },
  {
    name: 'Viral Fever', pct: 18, confidence: 'Medium',
    evidence: ['Elevated temperature', 'Fatigue and weakness', 'Upper respiratory symptoms'],
    risk: 'Moderate', icd: 'B34.9',
  },
  {
    name: 'Malaria', pct: 6, confidence: 'Low',
    evidence: ['Geographic risk area', 'Periodic fever pattern absent', 'Blood smear needed'],
    risk: 'High', icd: 'B54',
  },
  {
    name: 'Typhoid', pct: 4, confidence: 'Low',
    evidence: ['Rose spots not observed', 'Abdominal tenderness mild', 'Widal test needed'],
    risk: 'Moderate', icd: 'A01.0',
  },
]
...
const priorityColors = {
  Urgent: 'bg-red-100 text-red-600', High: 'bg-orange-100 text-orange-600', Medium: 'bg-yellow-100 text-yellow-600',
}
const urgencyColors = {
  Critical: 'bg-red-100 text-red-600 border-red-200', High: 'bg-orange-100 text-orange-600 border-orange-200',
}
  ...
  <AppShell>
    ...
    <div class="p-4 md:p-6 space-y-5">
      ...
      <div class="flex items-start gap-3 bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 text-xs text-amber-700">
        ...
        <span>This analysis is generated by AI and should be validated by a qualified healthcare professional before clinical decision-making. Confidence values represent probabilistic estimates, not definitive diagnoses.</span>
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          <div class="space-y-4">
            <div v-for="(c, i) in conditions" :key="c.name"
              ...
              <div class="h-2 bg-gray-100 rounded-full overflow-hidden mb-3">
                <div :class="[i===0?'bg-red-500':i===1?'bg-orange-400':i===2?'bg-yellow-400':'bg-gray-400', 'h-full rounded-full']" :style="`width:${c.pct}%`"></div>
              </div>
        ...
        <div class="space-y-4">
          ...
          <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
            ...
            <ul class="space-y-2">
              <li v-for="(t, i) in treatmentProtocol" :key="i" class="flex items-start gap-2.5 text-sm text-gray-700">
