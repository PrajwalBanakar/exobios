...
const filtered = computed(() => {
  ...
  return users.value.filter(u =>
    !q || u.name.toLowerCase().includes(q) || u.role.toLowerCase().includes(q) || u.area.toLowerCase().includes(q)
  )
})
...
function addUser() {
  ...
  users.value.push({
    ...
    ...newUser.value,
    status: 'Active', patients: 0, lastActive: 'Just now',
  })
...
<template>
  <AppShell>
    ...
    <div class="p-4 md:p-6 space-y-5">
      ...
      <!-- Success toast -->
      <div v-if="addSuccess"
        class="flex items-center gap-2 px-4 py-3 bg-green-50 border border-green-200 text-green-700 rounded-xl text-sm">
      ...
      <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div v-for="s in summaryStats" :key="s.labelKey"
          ...
          <div><div class="text-xl font-bold text-gray-900">{{ s.val }}</div><div class="text-xs text-gray-500">{{ t(s.labelKey) }}</div></div>
        </div>
      </div>
              ...
              <tr v-for="u in filtered" :key="u.id" class="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                ...
                <td class="px-4 py-3">
                  <div class="flex items-center gap-1">
                    ...
                    <button class="p-1.5 text-red-400 hover:bg-red-50 rounded transition" :title="t('common.delete')" @click="users = users.filter(x=>x.id!==u.id)">
