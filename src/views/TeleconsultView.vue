...
const filtered = computed(() => {
  let list = activeFilter.value === 'All'
    ? store.items
    : store.items.filter(s => s.status === activeFilter.value)
  if (search.value.trim()) {
    const q = search.value.toLowerCase()
    list = list.filter(s =>
      s.patientName.toLowerCase().includes(q) ||
      s.doctor.toLowerCase().includes(q) ||
      s.ashaWorker.toLowerCase().includes(q)
    )
  }
  return list
})
...
function filterLabel(f) {
  if (f === 'All') return t('teleconsult.allFilter')
...
function markCompleted(id) {
  store.updateStatus(id, 'Completed', 15)
}
...
<template>
  <AppShell>
    ...
    <div class="p-4 md:p-6 space-y-5">
      ...
      <!-- Scheduled Today panel -->
      <div v-if="store.scheduledCount > 0"
        class="bg-blue-600 rounded-xl p-4 flex items-center gap-4">
        ...
        <button @click="startVideoCall()"
          class="px-4 py-2 bg-white text-blue-600 text-xs font-semibold rounded-lg hover:bg-blue-50 transition flex-shrink-0">
          {{ t('teleconsult.join') }}
        </button>
      </div>
            ...
            <tbody>
              <tr v-for="s in filtered" :key="s.id"
                ...
                <td class="px-4 py-3.5 text-gray-600 text-sm">
                  {{ s.duration ? s.duration + ' ' + t('teleconsult.min') : '—' }}
                </td>
                <td class="px-4 py-3.5">
                  <span :class="[statusClasses[s.status] || 'bg-gray-100 text-gray-600', 'px-2.5 py-1 rounded-full text-xs font-semibold']">
                    {{ statusLabel(s.status) }}
                  </span>
                </td>
                <td class="px-4 py-3.5">
                  <div class="flex items-center gap-1">
                    <button v-if="s.status === 'Scheduled'"
                      @click="startVideoCall()"
                      class="flex items-center gap-1 px-2.5 py-1.5 bg-blue-600 hover:bg-blue-700 text-white text-xs font-semibold rounded-lg transition">
                    ...
                    <button v-if="s.status === 'Scheduled'"
                      ...
                      class="p-1.5 text-red-400 hover:bg-red-50 rounded transition"
                      :title="t('teleconsult.cancelled')">
