...
import { useTeleconsultStore } from '../stores/teleconsult'
...
const referral = reactive({
  required: 'no', center: 'Rampur Community Health Center', time: '', transport: 'Ambulance',
})

const teleconsult = reactive({
  done: 'no', doctor: 'Dr. Anjali Sharma', time: '', advice: '',
})
...
function handleFileChange(key, e) {
  ...
  if (file) evidenceFiles[key] = file
}
  ...
  try {
    ...
    if (referral.required === 'yes' && referral.center) {
      referralsStore.add({
        ...
        ashaWorker:  workerName,
        notes:       outcome.value.slice(0, 80),
      })
    }
    if (teleconsult.done === 'yes' && teleconsult.doctor) {
      teleconsultStore.add({
        patientId:   patientId.value,
        patientName: patient.value.name,
        doctor:      teleconsult.doctor,
        spec:        'General Physician',
        ashaWorker:  workerName,
        advice:      teleconsult.advice,
        status:      'Completed',
        duration:    teleconsult.time ? 15 : null,
      })
    }
  ...
  <AppShell>
    ...
    <div class="p-4 md:p-6 space-y-5">
      ...
      <div class="bg-white rounded-xl border border-gray-100 shadow-sm px-5 py-3 flex items-center gap-5 flex-wrap text-xs">
        ...
        <div>
          ...
          <div class="flex items-center gap-1">
            <span :class="['w-2 h-2 rounded-full', patient.risk === 'High' ? 'bg-red-500' : patient.risk === 'Moderate' ? 'bg-orange-400' : 'bg-green-500']"></span>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-5">
          ...
          <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-4">
            ...
            <div class="space-y-3">
              <div class="grid grid-cols-[1fr_auto_auto_1fr] gap-2 text-xs font-semibold text-gray-500 border-b border-gray-100 pb-2">
                <span>Action</span>
                <span class="text-center col-span-2">Done?</span>
                <span>Notes</span>
              </div>
            ...
            <div class="grid grid-cols-5 gap-2">
              <div v-for="slot in evidenceSlots" :key="slot.key" class="text-center">
                ...
                <div v-else class="border border-green-200 bg-green-50 rounded-lg p-2 relative">
                  <svg class="w-5 h-5 text-green-500 mx-auto mb-1" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                  ...
                  <button @click="removeFile(slot.key)"
                    class="absolute -top-1.5 -right-1.5 w-4 h-4 bg-red-500 text-white rounded-full flex items-center justify-center text-[9px] hover:bg-red-600">×</button>
                </div>
              </div>
            </div>
          </div>
        ...
        <div class="space-y-4">
          <!-- Patient Response — fix overflow with grid -->
          <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-4">
              ...
              <div v-if="referral.required === 'yes'" class="space-y-2.5">
                ...
                <div>
                  ...
                  <div class="flex gap-3 flex-wrap">
                    <label v-for="t in ['Ambulance','Private Vehicle','Govt. Vehicle','Walking']" :key="t"
      ...
      <div class="flex items-center justify-between pt-4 border-t border-gray-200">
        <div class="flex items-center gap-3">
          <button @click="router.back()"
            class="px-5 py-2.5 border border-gray-200 text-gray-600 text-sm font-medium rounded-lg hover:bg-gray-50 transition">Cancel</button>
