  ...
  <AppShell>
    ...
    <div class="p-4 md:p-6 space-y-5">
      ...
      <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div v-for="s in stats" :key="s.labelKey" class="bg-white rounded-xl border border-gray-100 shadow-sm p-4 flex items-center gap-3">
          ...
          <div class="flex items-end gap-3 h-40">
            <div v-for="d in monthlyData" :key="d.month" class="flex-1 flex flex-col items-center gap-1">
              <div class="w-full flex gap-1 items-end justify-center h-32">
                <div :style="`height: ${(d.cases / maxCases) * 100}%`" class="flex-1 bg-blue-500 rounded-t-sm min-h-[4px] transition-all"/>
                <div :style="`height: ${(d.referrals / maxCases) * 100}%`" class="flex-1 bg-green-400 rounded-t-sm min-h-[4px] transition-all"/>
              </div>
              <span class="text-[10px] text-gray-400">{{ d.month }}</span>
            </div>
          </div>
          <div class="flex items-center gap-4 mt-3 text-xs text-gray-500">
            <div class="flex items-center gap-1.5"><span class="w-3 h-3 rounded bg-blue-500"/><span>{{ t('reports.cases') }}</span></div>
            <div class="flex items-center gap-1.5"><span class="w-3 h-3 rounded bg-green-400"/><span>{{ t('reports.referrals') }}</span></div>
          </div>
        </div>
          ...
          <div class="mt-5">
            ...
            <div class="space-y-2">
              <div v-for="c in topConditions" :key="c.name" class="flex items-center gap-2 text-xs">
                <span :class="[c.color, 'w-2 h-2 rounded-full flex-shrink-0']"/>
