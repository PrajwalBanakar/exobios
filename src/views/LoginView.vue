...
async function verifyOtp() {
  ...
  if (code.length < 6 || otpDigits.value.some(d => d === '')) {
    otpError.value = 'Enter all 6 digits.'; return
  }
  ...
  try {
    await new Promise(r => setTimeout(r, 600))
    auth.login({ loginId: otpPhone.value, password: '' })
    await router.push('/dashboard')
  } catch {
...
function handleOtpInput(idx, e) {
  ...
  if (!otpLoading.value && otpDigits.value.every(d => d !== '') && otpDigits.value.join('').length === 6) {
    verifyOtp()
  }
}
...
async function handleSignup() {
  if (!signup.name || !signup.phone || !signup.ashaId || !signup.password) {
    signupError.value = 'Please fill all required fields.'; return
  }
...
function handleForgotOtpInput(idx, e) {
  ...
  if (val && idx < 5) {
    const next = document.getElementById(`fotp-${idx + 1}`)
    if (next) next.focus()
  }
}
      ...
      <div class="w-full max-w-sm">
        ...
        <template v-else-if="mode === 'otp'">
          ...
          <template v-else>
            ...
            <!-- Demo hint -->
            <p class="text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2 text-center mb-4">
              {{ t('login.demoHint') }}
            </p>
            ...
            <p class="text-center text-xs text-gray-400 mt-3">
              ...
              <button v-else @click="otpStep = 'phone'; otpError = ''" class="text-blue-600 hover:underline ml-1">
                {{ t('login.resend') }}
              </button>
            </p>
          </template>
        ...
        <template v-else-if="mode === 'forgot'">
          ...
          <template v-else-if="forgotStep === 'otp'">
            ...
            <button @click="verifyForgotOtp"
              class="w-full py-3 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-xl transition text-sm">
              {{ t('login.verify') }}
            </button>
          </template>
      ...
      <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[80vh] flex flex-col">
        <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <div class="flex gap-4">
            <button :class="['text-sm font-semibold pb-1 transition', termsTab === 'terms' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-500 hover:text-gray-700']"
              @click="termsTab = 'terms'">Terms of Service</button>
            <button :class="['text-sm font-semibold pb-1 transition', termsTab === 'privacy' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-500 hover:text-gray-700']"
              @click="termsTab = 'privacy'">Privacy Policy</button>
          </div>
        ...
        <div class="flex-1 overflow-y-auto px-6 py-5 text-sm text-gray-600 space-y-4 leading-relaxed">
          <template v-if="termsTab === 'terms'">
            ...
            <p>All clinical suggestions and recommendations generated by the AI are intended to assist, not replace, professional medical judgment. Final diagnosis and treatment decisions must be made by qualified healthcare professionals.</p>
            <h4 class="font-semibold text-gray-700">3. Patient Data</h4>
            <p>All patient data entered into the system is confidential. Users are responsible for maintaining the privacy and security of any patient information they access or record.</p>
            <h4 class="font-semibold text-gray-700">4. Account Responsibility</h4>
            <p>You are responsible for all activities that occur under your account. Do not share your login credentials with others.</p>
            <h4 class="font-semibold text-gray-700">5. Modifications</h4>
            <p>Exobios reserves the right to modify these terms at any time. Continued use of the platform after changes constitutes acceptance of the new terms.</p>
          </template>
          <template v-else>
            ...
            <p>We collect information necessary to provide healthcare assessment services, including user registration details and patient health information entered during assessments.</p>
            <h4 class="font-semibold text-gray-700">2. Data Use</h4>
            <p>Collected data is used exclusively to provide clinical decision support, generate AI recommendations, and improve patient care outcomes. We do not sell data to third parties.</p>
            <h4 class="font-semibold text-gray-700">3. Data Security</h4>
            <p>All data is encrypted in transit and at rest. We implement industry-standard security measures to protect patient information against unauthorized access.</p>
            <h4 class="font-semibold text-gray-700">4. Data Retention</h4>
            <p>Patient records are retained as required by applicable healthcare regulations. You may request deletion of your account data by contacting our support team.</p>
            <h4 class="font-semibold text-gray-700">5. Contact</h4>
