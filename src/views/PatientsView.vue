...
const avatarColors = {
  RK: 'bg-slate-200 text-slate-600', SD: 'bg-blue-100 text-blue-600',
  MS: 'bg-green-100 text-green-600', PK: 'bg-purple-100 text-purple-600',
  HY: 'bg-yellow-100 text-yellow-600',
}
const riskClasses = {
  High: 'bg-red-100 text-red-600', Moderate: 'bg-orange-100 text-orange-600', Low: 'bg-green-100 text-green-600',
}
const riskLabel = (r) => {
  if (r === 'High')     return t('risk.high')
  if (r === 'Moderate') return t('risk.moderate')
    ...
    <div class="p-6 space-y-5">
      ...
      <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div v-for="c in [
          ...
          <div><div class="text-xl font-bold text-gray-900">{{ c.val }}</div><div class="text-xs text-gray-500">{{ t(c.labelKey) }}</div></div>
        </div>
      </div>

      <div class="bg-white rounded-xl border border-gray-100 shadow-sm">
        <div class="flex items-center justify-between px-5 py-4 border-b border-gray-100 gap-3 flex-wrap">
          <h2 class="font-semibold text-gray-900">{{ t('users.allUsers').replace('Users', t('nav.patients')) }}</h2>
          <div class="flex items-center gap-3">
            ...
            <tbody>
              <template v-for="p in paged" :key="p.id">
                <tr v-if="deleteConfirmId !== p.id"
                  ...
                  @click="router.push(`/assessment/${p.id}/result`)">
                  <td class="px-5 py-3">
                    <div class="flex items-center gap-2.5">
                      <div :class="[avatarColors[p.initials] || 'bg-gray-100 text-gray-600', 'w-8 h-8 rounded-full flex items-center justify-center text-xs font-semibold flex-shrink-0']">{{ p.initials }}</div>
                      <div>
                  ...
                  <td class="px-4 py-3">
                    <span :class="[riskClasses[p.risk] || 'bg-gray-100 text-gray-600', 'px-2.5 py-1 rounded-full text-xs font-semibold']">{{ riskLabel(p.risk) }}</span>
                  </td>
                  <td class="px-4 py-3 text-gray-500 text-xs whitespace-nowrap">{{ p.date }}</td>
                  <td class="px-4 py-3">
                    <div class="flex items-center gap-1">
                      <button @click.stop="router.push(`/assessment/${p.id}/result`)" :title="t('common.viewAll')" class="p-1.5 text-gray-400 hover:bg-gray-100 rounded transition">
                      ...
                      <button @click.stop="router.push(`/assessment/${p.id}/edit`)" :title="t('common.edit')" class="p-1.5 text-blue-500 hover:bg-blue-50 rounded transition">
        ...
        <div class="flex items-center justify-between px-5 py-4">
          ...
          <div class="flex items-center gap-1">
            <button :disabled="currentPage===1" @click="goToPage(currentPage-1)"
              class="w-7 h-7 flex items-center justify-center text-gray-400 hover:bg-gray-100 rounded disabled:opacity-40">
